...
TOKEN = '6287829548:AAFIpHxRqdEPXUOcG4liVV7aDYsw8_cGUSg'
...
def welcome(message):
    ...
    markup.add(types.InlineKeyboardButton('Искать🎲', callback_data='get_first_anime'))
    reply_text = 'Привет!👋\nЭто бот для поиска рандомных аниме!\nСоздатель:@Akhmetov_Adil'
    bot.send_message(message.chat.id, reply_text, reply_markup=markup)
    cur.execute(f"""CREATE TABLE IF NOT EXISTS u{message.chat.id}(description text, main_text text, link text)""")
    db.commit()
...
def send_anime(message):
    try:
        ...
        try:
            ...
            try:
                bot.send_message(950479413, html.find('div', class_='anime-title').find('h1').text)
            except:
                pass
            r = requests.get('https://animego.org/anime/random')
            html = bs(r.text, 'lxml')
        ...
        #genres
        genres = ', '.join([x.text for x in html.find('dd', class_='col-6 col-sm-8 mb-1 overflow-h').findAll('a')])
        ...
        cur.execute(f"""CREATE TABLE IF NOT EXISTS u{message.chat.id}(description text, main_text text, link text)""")
        db.commit()
        cur.execute(f"""INSERT INTO u{message.chat.id} VALUES('{description}', '{main_text}', '{link}')""")
        db.commit()
        #send photo to user
...
def answ(call):
    if call.data == 'get_anime':
        ...
        try:
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text=description, reply_markup=markup)
        except:
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id,
                                  text='Нет', reply_markup=markup)
    elif call.data == 'back':
        ...
        markup1.add(types.InlineKeyboardButton('Дальше🔮', callback_data='get_anime'))
        markup1.add(types.InlineKeyboardButton('Описание🔍', callback_data='get_description'))
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text=main_text, reply_markup=markup1)
